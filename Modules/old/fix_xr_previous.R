fix_xr <- function(erdt, lcu_2_m49, eco_curr0, xrcountry){
  ...
  # Check when end dates do not match
    ...
    xr_corr[geographicaream49 == '58', end_year_eco_team := '2005']
    xr_corr[geographicaream49 == '58', start_year_eco := '1999']
    ...
    eco_curr[is.na(end_year) & active_flag == FALSE & country_code_m49 == '58', start_year := '1999']
    eco_curr[is.na(end_year) & active_flag == FALSE & country_code_m49 == '58', end_year := '2005']
    
    
    check <- merge(lcu_2_m49[end_year_iso >= '1990',.(code_m49, code_iso, start_year_iso, end_year_iso)], 
                   eco_curr[end_year >= '1990'], 
    ...
    prova <-  merge(problems, eco_curr[,.(country_code_m49, currency_code)], by.x = c('geographicAreaM49'),
                    by.y = c('country_code_m49'), all.x = T)
    tochange <- prova[timePointYears <= end_year & timePointYears >= start_year]
